...
kde_kwallet_password_get(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args,"ss|i",&realmstring,&username,&non_interactive)){
        PyErr_Clear();
        PyErr_SetString(PyExc_TypeError,"password_get() must be called as (service,username)");
        return NULL;
    }
...
kde_kwallet_password_set(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args,"sss|i",&realmstring,&username,&password,&non_interactive)){
        PyErr_Clear();
        PyErr_SetString(PyExc_TypeError,"password_set() must be called as (service,username,password)");
        return NULL;
    }
    if (non_interactive || (!dbus_bus_get(DBUS_BUS_SESSION,NULL))){
        PyErr_Clear();
