...
static const WCHAR description[] = L"python-keyring-lib.win32crpto";
...
crypto_encrypt(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sss|i", &realmstring, &username, &password, &non_interactive)){
        PyErr_Clear();
        PyErr_SetString(PyExc_TypeError,"encrypt() must be called as (servicename,username,passwrd)");                             return NULL;
    }
...
crypto_decrypt(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sss#|i", &realmstring, &username,&password_encrypted,&len_encrypted,
    &non_interactive)){
        PyErr_Clear();
        PyErr_SetString(PyExc_TypeError,"decrypt() must be called as (servicename,username,password)");                                                
        return NULL;                                                        
    }
