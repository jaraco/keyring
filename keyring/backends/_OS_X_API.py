...
class CFString(ctypes.Structure):

    def __str__(self):
        """
        Convert to a python string
        """
        str_length = CFStringGetLength(self) + 1
        str_buff = ctypes.create_string_buffer(str_length)
        assert CFStringGetCString(self, str_buff, str_length, 0)
        return str_buff.value.decode('ascii')
...
CFStringGetCString.argtypes = [CFStringRef, c_char_p, c_int32, c_int32]
